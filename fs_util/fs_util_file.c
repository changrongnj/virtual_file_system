...
int do_truncate(int inum, int len)
{
    ...
    int old_blk_len = (in->size -1) / FS_BLOCK_SIZE + 1;
    int new_blk_len = (len - 1) / FS_BLOCK_SIZE + 1;
    int current = old_blk_len;
	uint32_t buf[PTRS_PER_BLK], buf1[PTRS_PER_BLK];
    int i, j;
    int done = 0;
    int no_head_cleanup = 0;
    int no_head_cleanup_outer = 0;
            ...
            if (buf[i] != 0) {  // block allocated
                ...
                for (j = PTRS_PER_BLK - 1; j >= 0; j--) {
                    if (buf1[j] != 0) { // block allocated
                        if (new_blk_len == current) {
                            no_head_cleanup = 1;
                            no_head_cleanup_outer = 1;
                            done = 1;
                            break;
                        }
                ...
                disk->ops->write(disk, buf[i], 1, buf1);
                if (no_head_cleanup == 0) {
 ...
 * @return 0 if successful, or -error number
 */
int do_rename(int srcdir_inum, const char* src_leaf, int src_inum,
              int dstdir_inum, const char* dst_leaf, int dst_inum)
{
    if (srcdir_inum == 0) {
        return -EINVAL;
    }
    ...
    if (FS_VERSION == 1) {
        if (strcmp(src_leaf, ".") == 0 || strcmp(src_leaf, "..") == 0 || strcmp(dst_leaf, ".") == 0 || strcmp(dst_leaf, "..") == 0) {
            return -EINVAL;
        }
        struct fs_inode *src_inode = &fs.inodes[src_inum];
        // If target already exists and have same type, remove target first.
        if (dst_inum >= 0) {
            struct fs_inode *dst_inode = &fs.inodes[dst_inum];
            if (S_ISDIR(src_inode->mode) && S_ISDIR(dst_inode->mode)) {
                // target dir must be empty
                if (!is_dir_empty(dst_inum)) {
                    return -ENOTEMPTY;
                }
                do_rmdir(dstdir_inum, dst_leaf);
            } else if (S_ISREG(src_inode->mode) && S_ISREG(dst_inode->mode)) {
        ...
        if (S_ISREG(src_inode->mode)) {
            do_link(src_inum, dstdir_inum, dst_leaf);
            do_unlink(srcdir_inum, src_leaf);
        }
        if (S_ISDIR(src_inode->mode)) {
            do_link(src_inum, dstdir_inum, dst_leaf);
            do_unhardlink(srcdir_inum, src_leaf);
            do_unhardlink(src_inum, "..");
            do_link(dstdir_inum, src_inum, "..");
        }
        return 0;
    } else {
...
int do_unlink(int dir_inum, const char* leaf) {
    ...
    de[entno].valid = 0;
    disk->ops->write(disk, blkno, 1, buf);
    // If link count is not 0 after unlink, just return
    if(decrement_link_count(inum) == 0) {
        return 0;
    }
...
/**
 * Remove an hardlink named leaf in the specified
 * directory inum.
 *
 * Errors
 *   -ENOENT   - file does not exist
 ...
 * @return 0 if successful, or -error number
 */
int do_unhardlink(int dir_inum, const char* leaf) {
    // ensure inode is a directory
    struct fs_inode *din = &fs.inodes[dir_inum];
    if (!S_ISDIR(din->mode)) {
        return -ENOTDIR;
 ...
 *   -ENOSPC   - no free directory entry
 *
 * @param inum the inumber of the source file inode
 * @param dir_inum the directory inode of the target
 * @param leaf the name of the hard link in the directory path
 * @return 0 if successful, or -error number
 */
int do_link(int inum, int dir_inum, const char* leaf) {
    struct fs_inode *din = &fs.inodes[dir_inum];
    // make sure entry does not exist in directory
    int blkno;
    char buf[FS_BLOCK_SIZE];
    ...
    //update timestamp of source inode
    update_time(inum);
    mark_inode(inum);
...
/**
 * Increment the number of links of file
 *
 * Errors
 *
 * @param inum the inumber of the file inode
 * @return 0 if successful, or -error number
 */
...
/**
 * Decrement the number of links of file
 *
 * Errors
 *    -EPERM Link number is falling below 0
 *
 * @param inum the inumber of the file inode
 * @return 0 if successful, or -error number
 */
int decrement_link_count(int inum) {
    struct fs_inode *din = &fs.inodes[inum];
    // If the nlink is already 0, just return error
    if (din->nlink == 0) {
        return -EPERM;
    }
    din->nlink--;
    // If the nlink is 0, just return error
    if (din->nlink == 0) {
        return -EPERM;
    }
    mark_inode(inum);
...
/**
 * Update the ctime/mtime of file to latest
 *
 * Errors
 *    -EPERM Link number is falling below 0
 *
 ...
 * @return 0 if successful, or -error number
 */
int update_time(int inum) {
    struct fs_inode *din = &fs.inodes[inum];
    din->ctime = din->mtime = time(NULL);  // OK thorough 2100
    mark_inode(inum);
    // flush dirty metadata blocks
    flush_metadata();
    return 0;
